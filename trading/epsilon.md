...
Resting orders don't execute themselves. The **Matcher** is Epsilon's execution engine: it watches trigger conditions and, when one is met, fills the order through the meta-aggregator — the same best-route logic as a live swap. You pay gas only at order creation and cancellation; execution gas is the Matcher's problem.

## Fees

Every trade Epsilon executes — swap or resting order — pays a platform fee tiered by asset class, plus the flat **0.05% Matcher execution fee**, on top of the underlying pool fees of whichever venues the route touches:
