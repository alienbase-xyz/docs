...
> *Last updated: July 6, 2026.* Mothership is **pre-launch** as of the time of writing — audit in flight, alpha imminent. Everything below describes the design; details may shift before launch. <!-- TODO:USER on launch: add screenshots, deployed contract addresses, and a deposit walkthrough -->

## What Mothership is

A modular ALM with a **strategy-plugin architecture**. Each strategy is a smart contract that implements a defined interface (e.g., "manage a multi-range V3 position based on signal X"). Mothership routes user deposits into the strategy of their choice, and the strategy actively manages the underlying V3 NFT(s) on the user's behalf.

Concretely:

- **Multi-range positions.** A single Mothership strategy can hold multiple V3 ranges simultaneously, weighted dynamically.
- **Auto-rebalance.** When the price drifts toward a range boundary, the strategy re-positions without the user touching anything.
- **Signal-driven.** Strategies can use volatility data, time-of-day patterns, or external signals (oracles) to allocate capital.
- **Composable.** Mothership shares are ERC-20 — same composability story as Bunni.
